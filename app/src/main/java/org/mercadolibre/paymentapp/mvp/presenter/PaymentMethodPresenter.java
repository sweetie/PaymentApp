...
public class PaymentMethodPresenter
    ...
    public void getPaymentMethods() {
        Call<ResponseBody> call = Client.getRestAPIService().getPaymentMethods(Constants.API_KEY);
        ...
        call.enqueue(new Callback<ResponseBody>() {
            ...
            public void onResponse(Call<ResponseBody> call, Response<ResponseBody> response) {
                if (response.isSuccessful()) {
                    if (listener != null)
                        try {
                            listener.processResponse(Client.getAPIService(response.body().string(), ModelPaymentMethod[].class));
                        } catch (IOException e) {
                            e.printStackTrace();
                        }
                } else {
